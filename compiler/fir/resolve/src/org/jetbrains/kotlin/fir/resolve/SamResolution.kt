...
abstract class FirSamResolver {
    abstract fun getFunctionTypeForPossibleSamType(type: ConeKotlinType): SAMInfo<ConeKotlinType>?
...
class FirSamResolverImpl(
    ...
    override fun getFunctionTypeForPossibleSamType(type: ConeKotlinType): SAMInfo<ConeKotlinType>? {
        return when (type) {
            is ConeClassLikeType -> getFunctionTypeForPossibleSamType(type.fullyExpandedType(session))
            is ConeFlexibleType -> {
                val (lowerSymbol, lowerType) = getFunctionTypeForPossibleSamType(type.lowerBound) ?: return null
                val (_, upperType) = getFunctionTypeForPossibleSamType(type.upperBound) ?: return null
